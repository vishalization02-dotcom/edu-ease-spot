...
function DashboardPage() {
  ...
  const fees = useQuery({
    queryKey: ["fees", "month", month],
    queryFn: () => fetchFees({ month }),
  });
  const attendance = useQuery({
    queryKey: ["attendance", "day", today],
    queryFn: () => fetchAttendance({ date: today }),
  });
  ...
  const pendingAmount = (() => {
    ...
    const paidMap = new Map(
      fees.data.filter((f) => f.status === "paid").map((f) => [f.student_id, true]),
    );
    return students.data
      .filter((s) => !paidMap.get(s.id))
      .reduce((sum, s) => sum + Number(s.monthly_fee || 0), 0);
  })();
  const todayPresent = (attendance.data ?? []).filter(
    (a) => a.date === today && a.status === "present",
  ).length;
  const todayAbsent = (attendance.data ?? []).filter(
  ...
  const topClasses = useMemo(() => {
    ...
    list.sort(
      (a, b) => (studentCountByClass.get(b.id) ?? 0) - (studentCountByClass.get(a.id) ?? 0),
    );
  ...
  const pendingByClass = useMemo(() => {
    const paidSet = new Set(
      (fees.data ?? []).filter((f) => f.status === "paid").map((f) => f.student_id),
    );
      ...
      <div className="space-y-0.5 -mt-1">
        ...
        <div className="flex items-start justify-between">
          <div className="-mt-1 space-y-1">
            <h1 className="text-2xl font-semibold tracking-tight">
              {greeting}
              {profile.data?.full_name ? `, ${profile.data.full_name.split(" ")[0]}` : ""}
              👋
            </h1>

            <p className="text-sm text-muted-foreground">Ready to inspire your students today.</p>
          </div>
        ...
        <div className="flex flex-col gap-1 lg:flex-row">
          <div className="lg:w-2/3">
            <BusinessOverview collected={collected} pending={pendingAmount} />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
            {topClasses.map((c) => {
              ...
              const attendancePercentage =
                totalAttendance === 0 ? 0 : Math.round((att.present / totalAttendance) * 100);
              return (
                  ...
                  <Card className="group h-full w-full rounded-2xl border border-border/60 bg-card/50 p-4 transition-all duration-300 hover:-translate-y-1 hover:border-primary/40 hover:bg-card hover:shadow-xl cursor-pointer">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-xs text-muted-foreground">
                        <div className="truncate text-[15px] font-semibold"></div>
                    ...
                    <div className="flex items-center justify-between">
                      <span className="font-medium text-muted-foreground">Pending Fees</span>
                      <span
                        className={`font-semibold ${
                          pending > 0 ? "text-amber-500" : "text-emerald-500"
                        }`}
                      >
  ...
  return (
    <Card className="group h-full rounded-2xl border border-border/60 bg-card/50 p-4 transition-all duration-200 hover:-translate-y-0.5 hover:border-primary/40 hover:bg-card hover:shadow-lg cursor-pointer">
      <div className="flex items-start justify-between">
        ...
        <div
          className={`h-10 w-10 rounded-lg grid place-items-center text-lg ${toneBg}`}
          aria-hidden
        >
          <span>{emoji}</span>
        </div>
...
function RecentActivity() {
  ...
  const att = useQuery({
    ...
    queryFn: () => fetchAttendance({ limit: 20 }),
  });
  ...
  const items: Item[] = [];
  (fees.data ?? [])
    .filter((f) => f.status === "paid" && f.payment_date)
    .forEach((f) => {
  ...
  if (top.length === 0)
    return (
      <div className="text-sm text-muted-foreground">
        No activity yet. Start by adding a student.
      </div>
    );
          ...
          >
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="mt-0.5 text-sm text-muted-foreground">{i.subtitle}</p>
              </div>
            </div>
