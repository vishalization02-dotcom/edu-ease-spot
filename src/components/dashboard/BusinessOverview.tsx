...
export function BusinessOverview({ collected, pending }: BusinessOverviewProps) {
  const expected = collected + pending;

  const percentage = expected === 0 ? 0 : Math.round((collected / expected) * 100);
    ...
    <Card className="rounded-2xl border border-border/60 bg-card px-5 py-4">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-base text-muted-foreground">Track your coaching income at a glance.</p>
        </div>
      </div>
      ...
      <div className="mt-4 grid grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-0 lg:divide-x lg:divide-border/50">
        <div className="pr-5">
          ...
          <p className="mt-2 text-3xl font-bold text-emerald-400">₹{collected.toLocaleString()}</p>
        ...
        <div className="px-5">
          ...
          <p className="mt-2 text-3xl font-bold text-red-400">₹{pending.toLocaleString()}</p>
        ...
        <div className="px-5">
          ...
          <p className="mt-2 text-3xl font-bold text-blue-400">₹{expected.toLocaleString()}</p>

          <p className="mt-2 flex items-center gap-1">This Month</p>
        </div>

        <div className="pl-5">
          <p className="text-sm text-muted-foreground">Collection Rate</p>

          <p className="mt-2 text-3xl font-bold text-violet-400">{percentage}%</p>

          <p className="mt-2 text-sm text-muted-foreground">of expected income</p>
        </div>
      </div>
